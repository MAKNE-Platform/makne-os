"use server";

import mongoose from "mongoose";
import { cookies } from "next/headers";
import { redirect } from "next/navigation";
import { connectDB } from "@/lib/db/connect";
import { Agreement } from "@/lib/db/models/Agreement";

export async function saveDeliverablesAction(formData: FormData) {
  const cookieStore = await cookies();
  const brandId = cookieStore.get("auth_session")?.value;
  const role = cookieStore.get("user_role")?.value;
  const agreementId = cookieStore.get("draft_agreement_id")?.value;

  if (!brandId || role !== "BRAND") {
    redirect("/auth/login");
  }

  if (!agreementId) {
    redirect("/brand/dashboard");
  }

  const titles = formData.getAll("deliverable_title") as string[];
  const descriptions = formData.getAll("deliverable_description") as string[];

  const deliverables = titles
    .map((title, index) => ({
      _id: new mongoose.Types.ObjectId(),
      title,
      description: descriptions[index] || undefined,
    }))
    .filter((d) => d.title.trim().length > 0);

  if (deliverables.length === 0) {
    throw new Error("At least one deliverable is required");
  }

  await connectDB();

  await Agreement.findOneAndUpdate(
    {
      _id: new mongoose.Types.ObjectId(agreementId),
      brandId: new mongoose.Types.ObjectId(brandId),
      status: "DRAFT",
    },
    {
      deliverables,
      $push: {
        activity: { message: "Deliverables defined" },
      },
    }
  );

  redirect("/agreements/create/milestones");
}
